...
struct _PerformanceThreadMapHash {
    size_t operator()(const NSInvocation *s1) const { return (size_t)s1; }
};

struct _PerformanceThreadMapEqual {
...
} PerformanceThreadInvocParams;


/////////////////////////////////////////////////////////////////////////
#pragma mark - PerformanceThread
/////////////////////////////////////////////////////////////////////////

@implementation PerformanceThread
{
    std::unordered_map<NSInvocation *, PerformanceThreadInvocParams, _PerformanceThreadMapHash, _PerformanceThreadMapEqual >_invocationsDict; ///< The time repeat interval which to call the methods.  One of these needs to be retains to why not this one
    
    std::unordered_map<NSInvocation *, NSTimeInterval, _PerformanceThreadMapHash, _PerformanceThreadMapEqual >_invocationsToAddDict;
    
    std::unordered_set<NSInvocation *, _PerformanceThreadMapHash > _invocationsToRemoveSet; ///< Temp hold invokes sent to removeInvocation to remove when run loop is finished
...
- (void)addInvocation:(NSInvocation *)invocation desiredInterval:(NSTimeInterval)timeInterval
{
    ...
    if (self.isExecuting && (!self.paused || runLoopCoreIsExecuting)) {
        ...
        @synchronized(_invocationsDictMutex) {
            PerformanceThreadInvocParams tmp = {0};
